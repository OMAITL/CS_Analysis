...
class SearchService:
    ...
    @staticmethod
    def _cs_weapon_token(market_hash_name: str) -> str:
        """Extract weapon/collection token from market_hash_name for search queries."""
    ...
    @staticmethod
    def _cs_tieba_keywords(item_name: str, weapon: str) -> str:
        """Short Chinese keywords for Tieba (avoid full hash-name breaking search)."""
        ...
        for part in raw.replace("(", " ").replace(")", " ").split("|"):
            ...
            if part and part not in tokens:
                tokens.append(part)
        tokens.extend(["CS2", "饰品"])
        return " ".join(tokens[:4])

    _CS_EVENT_REQUIRED_DOMAINS: Dict[str, Tuple[str, ...]] = {
        "official_update": ("blog.counter-strike.net",),
        "new_case": ("blog.counter-strike.net",),
        "hltv": ("hltv.org",),
        "tieba": ("tieba.baidu.com",),
    }

    _CS_EVENT_NOISE_TERMS: Tuple[str, ...] = (
        "nba finals",
        "nhl",
        "stanley cup",
        "super bowl",
    ...
    _CS_EVENT_CORE_TERMS: Tuple[str, ...] = (
        "cs2",
        "csgo",
        "counter-strike",
        "counter strike",
    ...
    def _pick_cs_event_provider(
        self,
        dimension: str,
        available_providers: List[BaseSearchProvider],
    ) -> BaseSearchProvider:
        """Prefer engines that handle ``site:`` / domain search better."""
        preference = {
            "item_focus": ("Bocha", "Tavily", "SerpAPI", "Brave"),
            "case_focus": ("Bocha", "Tavily", "SerpAPI", "Brave"),
            "official_update": ("SerpAPI", "Brave", "Tavily", "Bocha"),
            "new_case": ("SerpAPI", "Brave", "Tavily", "Bocha"),
            "hltv": ("SerpAPI", "Brave", "Tavily"),
            "major_sticker": ("SerpAPI", "Tavily", "Brave", "Bocha"),
            "cs_market": ("Bocha", "Tavily", "SerpAPI", "Brave"),
            "tieba": ("Bocha", "SerpAPI", "Tavily", "Brave"),
        }.get(dimension, ())
    ...
    def _filter_cs_event_relevance(
        self,
        response: SearchResponse,
        *,
        dimension: str,
        ...
        """Drop off-topic hits; enforce domain rules for site-scoped dimensions."""
        if not response.success or not response.results:
            return response

        required_domains = self._CS_EVENT_REQUIRED_DOMAINS.get(dimension)
        ...
        for result in response.results:
            ...
            if required_domains and not any(dom in url_lower for dom in required_domains):
                continue

            if any(noise in blob for noise in self._CS_EVENT_NOISE_TERMS):
                continue

            if dimension in ("major_sticker", "tieba") and not required_domains:
                has_cs = any(term in blob for term in self._CS_EVENT_CORE_TERMS)
                has_item = bool(weapon_lower and weapon_lower in blob) or any(
                    tok in blob for tok in item_lower.split() if len(tok) >= 2
                )
                if not has_cs and not has_item:
                    continue

            if dimension == "tieba" and required_domains:
                has_cs = any(term in blob for term in self._CS_EVENT_CORE_TERMS)
                has_item = bool(weapon_lower and weapon_lower in blob)
                if not has_cs and not has_item:
                    continue

            if dimension == "cs_market":
                has_cs = any(term in blob for term in self._CS_EVENT_CORE_TERMS)
                has_item = bool(weapon_lower and weapon_lower in blob) or any(
                    tok in blob for tok in item_lower.split("|") if len(tok.strip()) >= 2
                )
                if not has_cs and not has_item:
                    continue

            if dimension == "item_focus":
                has_item = bool(weapon_lower and weapon_lower in blob) or any(
                    tok.strip().lower() in blob
                    for tok in item_name.replace("|", " ").replace("(", " ").split()
                    if len(tok.strip()) >= 2
                )
                has_cs = any(term in blob for term in self._CS_EVENT_CORE_TERMS)
                if not has_item and not has_cs:
                    continue

            if dimension == "case_focus":
                labels = [str(label).strip().lower() for label in (case_labels or []) if label]
                has_case = any(label in blob for label in labels)
                has_cs_case = any(
                    term in blob for term in ("武器箱", "weapon case", " case ", "开箱", "drop pool")
                )
                has_cs = any(term in blob for term in self._CS_EVENT_CORE_TERMS)
                if not has_case and not (has_cs_case and has_cs):
                    continue
        ...
        if not kept and required_domains:
            for result in response.results:
                blob = f"{result.title} {result.snippet}".lower()
                if any(noise in blob for noise in self._CS_EVENT_NOISE_TERMS):
                    continue
                if any(term in blob for term in self._CS_EVENT_CORE_TERMS):
                    kept.append(result)
                if len(kept) >= max_results:
                    break
    ...
    def search_cs_event_intel(
        ...
        """
        Multi-dimension event intel for CS2 skins (official / Major / cases / HLTV / Tieba).

        Mirrors stock ``search_comprehensive_intel`` but with CS-specific queries.
        """
        weapon = self._cs_weapon_token(market_hash_name)
        name = (item_name or "").strip() or str(good_id)
        tieba_kw = self._cs_tieba_keywords(name, weapon)
        _, skin_zh = name.split("|", 1) if "|" in name else ("", name)
        skin_zh = skin_zh.split("(")[0].strip()
        _, skin_en = market_hash_name.split("|", 1) if "|" in market_hash_name else ("", "")
        skin_en = skin_en.split("(")[0].strip()
        item_focus_q = " ".join(
            part
            for part in (
                f'"{market_hash_name}"',
                f"{weapon} {skin_en} CS2 skin price",
                f"{skin_zh or name} CS2 饰品 价格",
                tieba_kw,
            )
            if part
        )
        ...
        case_query = build_case_search_query(case_labels or [])
        if case_query:
            search_dimensions.append(
                {
                    "name": "case_focus",
                    "query": f"{case_query} 武器箱 价格 掉落",
                    "desc": "所属武器箱",
                    "tavily_topic": "news",
                    "strict_freshness": False,
                }
        ...
        search_dimensions.extend(
            [
            {
                "name": "official_update",
                "query": "site:blog.counter-strike.net Counter-Strike 2 update",
                "desc": "官方更新",
                "tavily_topic": "news",
                "strict_freshness": True,
            },
            {
                "name": "new_case",
                "query": "site:blog.counter-strike.net Counter-Strike 2 case capsule",
                "desc": "新箱子",
                "tavily_topic": "news",
                "strict_freshness": True,
            },
            {
                "name": "hltv",
                "query": "site:hltv.org/news Counter-Strike 2",
                "desc": "HLTV新闻",
                "tavily_topic": None,
                "strict_freshness": True,
            },
            {
                "name": "cs_market",
                "query": f"CS2 饰品 市场 新箱子 贴纸 {weapon} OR CS2 skin case sticker market",
                "desc": "饰品市场",
                "tavily_topic": "news",
                "strict_freshness": False,
            },
            {
                "name": "major_sticker",
                "query": "site:hltv.org CS2 Major sticker capsule",
                "desc": "Major贴纸",
                "tavily_topic": None,
                "strict_freshness": True,
            },
            {
                "name": "tieba",
                "query": f"site:tieba.baidu.com {tieba_kw}",
                "desc": "贴吧讨论",
                "tavily_topic": None,
                "strict_freshness": False,
            },
        ...
        search_count = 0
        provider_index = 0
        code_label = f"cs:{good_id}"

        logger.info(
            "开始 CS 事件情报搜索: %s (%s), 时间范围: 近%s天, max_searches=%s",
            name,
            code_label,
            search_days,
            max_searches,
        ...
        for dim in search_dimensions:
            ...
            provider = self._pick_cs_event_provider(dim["name"], available_providers)
            provider_index += 1
            logger.info("[CS情报] %s: 使用 %s, query=%s", dim["desc"], provider.name, dim["query"])
            ...
            if dim["strict_freshness"]:
                filtered_response = self._filter_news_response(
                    ...
                    search_days=search_days,
                    max_results=provider_max_results,
                    log_scope=f"{code_label}:{provider.name}:{dim['name']}",
                )
            else:
                filtered_response = self._normalize_and_limit_response(
                    response,
            ...
            filtered_response = self._filter_cs_event_relevance(
                filtered_response,
                dimension=dim["name"],
                item_name=name,
                weapon=weapon,
    ...
    def format_cs_event_report(
        self,
        intel_results: Dict[str, SearchResponse],
        item_name: str,
    ) -> str:
        """Format CS event intel search results for LLM / reports."""
        lines = [f"【{item_name} CS 事件与舆论情报】"]
        display_order = [
            "item_focus",
            "case_focus",
            "official_crawl",
