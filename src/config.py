...
class Config:
    ...
    cs_event_intel_enabled: bool = True  # CS 分析时检索事件情报（官方/Major/贴吧等）
    cs_event_intel_max_searches: int = 8
    cs_news_strategy_profile: str = "medium"  # CS 事件情报窗口档位（建议 medium/long）
    hltv_crawl_enabled: bool = False  # 低频率抓取 HLTV 新闻 sitemap（无官方 API，默认关闭）
    hltv_crawl_max_items: int = 4
    cs_official_feed_enabled: bool = True  # Counter-Strike 官方博客 RSS
    cs_official_feed_max_items: int = 5
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        return cls(
            ...
            cs_event_intel_enabled=os.getenv('CS_EVENT_INTEL_ENABLED', 'true').lower() == 'true',
            cs_event_intel_max_searches=parse_env_int(
                os.getenv('CS_EVENT_INTEL_MAX_SEARCHES'),
                7,
                field_name='CS_EVENT_INTEL_MAX_SEARCHES',
                minimum=1,
            ),
            cs_news_strategy_profile=cls._parse_news_strategy_profile(
                os.getenv('CS_NEWS_STRATEGY_PROFILE', 'medium')
            ),
            hltv_crawl_enabled=os.getenv('HLTV_CRAWL_ENABLED', 'false').lower() == 'true',
            hltv_crawl_max_items=parse_env_int(
                os.getenv('HLTV_CRAWL_MAX_ITEMS'),
                4,
                field_name='HLTV_CRAWL_MAX_ITEMS',
                minimum=1,
            ),
            cs_official_feed_enabled=os.getenv('CS_OFFICIAL_FEED_ENABLED', 'true').lower() == 'true',
            cs_official_feed_max_items=parse_env_int(
                os.getenv('CS_OFFICIAL_FEED_MAX_ITEMS'),
                5,
                field_name='CS_OFFICIAL_FEED_MAX_ITEMS',
                minimum=1,
            ),
