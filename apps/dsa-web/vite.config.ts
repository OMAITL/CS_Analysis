...
export default defineConfig(({ mode }) => {
  ...
  const apiHost = (env.WEBUI_HOST || env.VITE_API_HOST || '127.0.0.1').trim()
  const apiPort = Number(env.VITE_API_PORT || env.WEBUI_PORT || 8000)
  ...
  define: {
    ...
    __LOCAL_API_HOST__: JSON.stringify(apiHost),
    __LOCAL_API_PORT__: JSON.stringify(String(apiPort)),
  },
  plugins: [
    react({
      babel: {
