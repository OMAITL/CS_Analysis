...
declare const __LOCAL_API_HOST__: string | undefined;
declare const __LOCAL_API_PORT__: string | undefined;
...
/** Dev-only: injected from root `.env` (`WEBUI_HOST` / `WEBUI_PORT`) via Vite. */
export const LOCAL_DEV_API_HOST = typeof __LOCAL_API_HOST__ === 'string' && __LOCAL_API_HOST__.trim()
  ? __LOCAL_API_HOST__.trim()
  : '127.0.0.1';
export const LOCAL_DEV_API_PORT = typeof __LOCAL_API_PORT__ === 'string' && __LOCAL_API_PORT__.trim()
  ? __LOCAL_API_PORT__.trim()
  : '8000';
export const LOCAL_DEV_API_ORIGIN = `http://${LOCAL_DEV_API_HOST}:${LOCAL_DEV_API_PORT}`;

/** CS 单品分析（事件情报 + LLM）常超过 2 分钟，前端需更长等待。 */
export const CS_ITEM_ANALYZE_TIMEOUT_MS = 300_000;
