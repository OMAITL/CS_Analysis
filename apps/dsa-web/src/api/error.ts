...
import { LOCAL_DEV_API_ORIGIN, LOCAL_DEV_API_PORT } from '../utils/constants';
...
export function parseApiError(error: unknown): ParsedApiError {
  ...
  if (includesAny(matchText, ['timeout', 'timed out', 'read timeout', 'connect timeout']) || code === 'ECONNABORTED') {
    const reqUrl = getRequestUrl(error);
    const clientWaitTimeout = code === 'ECONNABORTED' && !response;
    if (clientWaitTimeout && reqUrl.includes('/cs/items/analyze')) {
      return createParsedApiError({
        title: '分析等待超时',
        message:
          'CS 分析含事件情报与 LLM，完整流程常需 2–3 分钟。请重新点击「分析」并耐心等待；若仍偏慢，可在根目录 .env 设置 CS_EVENT_INTEL_ENABLED=false 跳过情报搜索。',
        rawMessage,
        status,
        category: 'upstream_timeout',
      });
    }
    if (clientWaitTimeout) {
      return createParsedApiError({
        title: '请求等待超时',
        message: '前端等待后端响应超时，请确认后端仍在运行后重试。',
        rawMessage,
        status,
        category: 'upstream_timeout',
      });
